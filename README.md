...
```bash
performance-testing-tool$ sam build --use-container
```
...
```bash
performance-testing-tool$ sam logs -n StockCheckerFunction --stack-name performance-testing-tool --tail
```
...
```bash
performance-testing-tool$ pip install -r tests/requirements.txt --user
# unit test
performance-testing-tool$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
performance-testing-tool$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name performance-testing-tool
```
